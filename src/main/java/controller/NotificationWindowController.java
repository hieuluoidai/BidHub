...
public class NotificationWindowController {
    ...
    private Consumer<Notification> onMarkOne;
    private Consumer<Notification> onAction;
        ...
        listNotifications.setCellFactory(lv -> new ListCell<>() {
            ...
            protected void updateItem(Notification n, boolean empty) {
                ...
                if (empty || n == null) {
                    setGraphic(null);
                    setText(null);
                    getStyleClass().remove("notification-cell-read");
                    getStyleClass().remove("notification-cell-unread");
                    setStyle("-fx-background-color: transparent;");
                } else {
                    setGraphic(buildRow(n));
                    getStyleClass().removeAll("notification-cell-read", "notification-cell-unread");
                    getStyleClass().add(n.isRead() ? "notification-cell-read" : "notification-cell-unread");
    ...
    public void setOnMarkOne(Consumer<Notification> c) {
        this.onMarkOne = c;
    }
    public void setOnAction(Consumer<Notification> c) {
        this.onAction = c;
    }
    ...
    @FXML
    void handleClose() {
        ((Stage) listNotifications.getScene().getWindow()).close();
    }

    private VBox buildRow(Notification n) {
        VBox row = new VBox(6);
        row.getStyleClass().add("notification-window-row");
        if (!n.isRead()) row.getStyleClass().add("notification-window-row-unread");
        ...
        title.getStyleClass().add("notification-window-row-title");
        
        Label time = new Label(formatRelative(n.getCreatedAt()));
        time.getStyleClass().add("notification-window-row-time");
        titleArea.getChildren().addAll(title, time);

        Region spacer = new Region();
        HBox.setHgrow(spacer, javafx.scene.layout.Priority.ALWAYS);

        top.getChildren().addAll(iconNode, titleArea, spacer);

        Label msg = new Label(n.getMessage() == null ? "" : n.getMessage());
        msg.setWrapText(true);
        msg.getStyleClass().add("notification-window-row-msg");
    ...
    private Node createIcon(Notification.Type t) {
        String pathData = switch (t) {
            ...
            case AUCTION_WON ->
                ...
                + "v-2h2c3.31 0 6-2.69 6-6V4c0-1.1-.9-2-2-2zm0 12h-2c-1.1 0-2-.9-2-2V4h4v10zM3 4h3v7c0 1.66 1.34 3 3 3H3V4z";
        ...
        pane.getStyleClass().add("notification-window-icon");
